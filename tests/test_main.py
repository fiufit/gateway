...
def test_successful_register(mock_make_request):
    ...
    response = client.post("/v1/users/register", json={"email": "test@example.com", "password": "testpassword"})
...
def test_incorrect_email_format_in_body_register():
    response = client.post("/v1/users/register", json={"e-mail": "test@example.com", "password": "testpassword"})
...
def test_incorrect_password_format_in_body_register():
    response = client.post("/v1/users/register", json={"email": "test@example.com", "pw": "testpassword"})
    assert response.status_code == 400
    response_body = response.json()
    assert response_body['error']['code'] == 'G1'
...
def test_incorrect_email_and_password_format_in_body_register():
    response = client.post("/v1/users/register", json={"e-mail": "test@example.com", "pw": "testpassword"})
    assert response.status_code == 400
    response_body = response.json()
    assert response_body['error']['code'] == 'G1'
...
def test_finish_register_without_authorization_header():
    ...
    response = client.post("/v1/users/finish-register", json={"name": "Test User"}, headers=headers)
...
def test_successful_finish_register(mock_bearer_class, mock_make_request):
    mock_make_request.return_value = {"success": True}
    mock_bearer_class.return_value = {"name":"Lionel Messi", "uid":"fake_uid", "email_verified":True}
